...
class JoshController extends Controller
{
	...
	*/
    ...
    public function showHome()
    {
		...
		$businesses_count = PaySubscription::count();
		$businesses_active = PaySubscription::where('pay_status', 1)
		                                      ->count();

        $trialcount = PaySubscription::where('pay_status', 0)  //double check this
		                                      ->count();
        /*$all = Countries::all();
		print_r($all);*/
        $countries_to_plot = array();
	     $all_contries = CountriesRepository::countries();
		 foreach($all_contries as $country){
               $count_users_from_country = User::where('country', $country['code2l'])->count();
               if($count_users_from_country){
                   $country['number_of_users'] = $count_users_from_country;
                   array_push($countries_to_plot, $country);
               }
		 }

		 /*print_r($countries_to_plot);
        die;*/
											  
    	if(Sentinel::check())
			return view('admin.index', compact('total_users', 'businesses_count', 'businesses_active', 'trialcount', 'countries_to_plot'));
		else
